...
class GeofenceTransitionsJobIntentService : JobIntentService(), CoroutineScope {
    ...
    override fun onHandleWork(intent: Intent) {
        ...
        if (geofencingEvent.hasError()) {
            ...
            val error = when (geofencingEvent.errorCode) {
                ...
                GeofenceStatusCodes.GEOFENCE_TOO_MANY_PENDING_INTENTS -> getString((R.string.geofence_not_available))
                else -> getString(R.string.geofence_unknown_error)

            }
            Timber.i(getString(R.string.geoservice_error), error)

        }

        when(geofencingEvent.geofenceTransition){
            Geofence.GEOFENCE_TRANSITION_ENTER -> sendNotification(geofencingEvent.triggeringGeofences)
        }
    }
