...
const val REQUEST_CODE_BACKGROUND = 201
const val REQUEST_FOREGROUND_AND_BACKGROUND_PERMISSION_RESULT_CODE = 33
const val REQUEST_FOREGROUND_ONLY_PERMISSIONS_REQUEST_CODE = 34
const val BACKROUND_LOCATION_PERMISSION_INDEX = 1
...
fun foregroundAndBackgroundLocationPermissionApproved(context: Context): Boolean {
    ...
    //if running Q or Highter check that the background access is provided
    val backgroundPermissionApproved =
        if (runningQOrLater) {
            ...
            //if the device is running lower than Q, than return true because  you dont need that
            // permission to run in the background
            true
        }

    // this will return true if the permission is granted and false if not
